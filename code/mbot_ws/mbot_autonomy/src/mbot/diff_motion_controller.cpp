...
public:
    ...
    virtual mbot_lcm_msgs::twist2D_t get_command(const mbot_lcm_msgs::pose2D_t& pose, const mbot_lcm_msgs::pose2D_t& target) override
    {
        float dx = target.x - pose.x;
        float dy = target.y - pose.y;
        // float d_theta = angle_diff(atan2(dy,dx-0.1), pose.theta) * 1.1;
        float d_theta = angle_diff(atan2(dy,dx), pose.theta)* 1.1;
        ...
        float turn_vel = turn_pid[0] * d_theta + turn_pid[1] + turn_pid[2] * turn_der;
        fprintf(stdout,"Turn error: %f\tTurn vel: %f\tPose theta: %f\n", d_theta, turn_vel, pose.theta);
        // if (turn_vel < 0.4) turn_vel = M_PI_4 /2.;
        // else if (turn_vel > -0.4) turn_vel = -M_PI_4/2.;
    ...
    virtual bool target_reached(const mbot_lcm_msgs::pose2D_t& pose, const mbot_lcm_msgs::pose2D_t& target, bool is_end_pose)  override
    {
        ...
        return (fabs(angle_diff(pose.theta, target_heading)) < 0.025);
    }
...
private:
    float pid[3] = {0.9, 6, 0.0}; //kp, ki, kb works for fast speed
    // float pid[3] = {2.0, 6, 0.0}; //kp, ki, kb!! works for SLOW SPEED  
    float d_end_crit = 0.02;
    float d_end_midsteps = 0.08;
    float angle_end_crit = 0.03;
...
public:
    ...
    virtual mbot_lcm_msgs::twist2D_t get_command(const mbot_lcm_msgs::pose2D_t& pose, const mbot_lcm_msgs::pose2D_t& target) override
    {
        ...
        float alpha = angle_diff(atan2(dy,dx), pose.theta); // difference in heading
        // printf("alpha: %f\n", alpha);
        if (first_alpha && fabs(alpha) >= 0.786)
        {
            alpha += (alpha > 0) ? 0.262 : ((alpha < 0) ? -0.262 : 0);
        }

        float dtheta  = fabs(target.theta - pose.theta);
        float d_theta_val = target.theta - pose.theta;

        // printf("current pose: x, y, theta: %f  %f  %f", pose.x, pose.y, pose.theta);
        // printf("\n");
        ...
        // if (fwd_last_error > 0)
        //     fwd_der = (d_fwd - fwd_last_error) / 0.05;
        // float fwd_vel = 1. * d_fwd + 0.5 * fwd_sum_error + pid[2] * fwd_der;

        float turn_vel = pid[1] * alpha + pid[2] * beta;
        // float turn_vel = pid[1] * d_theta_val + pid[2] * beta;
        // printf("turn_vel: %f\n", turn_vel);

        // printf("dtheta, alpha: %f %f", dtheta, alpha);
        printf("\n");
        if (fabs(alpha) > angle_end_crit)
        {
            fwd_vel = 0.0;
            turn_vel = turn_vel*1.15;
        }
    ...
    virtual bool target_reached(const mbot_lcm_msgs::pose2D_t& pose, const mbot_lcm_msgs::pose2D_t& target, bool is_end_pose)  override
    {   
        // printf("SMART CONTROLLER CHECKPOINT: checking if target reached\n");
        ...
        float target_heading = atan2(dy, dx);
        float angle_diff_check_val = fabs(angle_diff(pose.theta, target_heading));
        if (alpha > 0.786)
        {
           angle_diff_check_val+=0.2; 
        }

        bool check = ((fabs(pose.x - target.x) < distance) && (fabs(pose.y - target.y)  < distance) && (angle_diff_check_val < 0.025));

        // printf("reached bool: %d\n", check);
        return check;
    }
};
        ...
        if(!targets_.empty() && !odomTrace_.empty()) 
        {
            ...
            if (state_ == SMART) 
            {
                if (smart_controller.target_reached(pose, target, is_last_target))
                {
                    ...
                    // printf("TARGET NOT REACHED! new getting command\n");

                    // add extra rotation command here
                    float dx = target.x - pose.x;
                    float dy = target.y - pose.y;
                    float d_fwd = sqrt(dx * dx + dy * dy);
                    float alpha = angle_diff(atan2(dy,dx), pose.theta); // difference in heading
                    printf("alpha: %f\n", alpha);
                     cmd = smart_controller.get_command(pose, target);
                    // if (fabs(alpha) >= 0.045)
                    // {
                    //     // alpha += (alpha > 0) ? 0.262 : ((alpha < 0) ? -0.262 : 0);
                    //     ROTATE_MORE_FLAG = true;
                    //     state_ == INITIAL_TURN;
                    //     // printf("ROTATE_MORE_FLAG: %d\n", ROTATE_MORE_FLAG);
                    //     printf("using TURN controller\n");
                    //     cmd = turn_controller.get_command(pose, target);
                    //     // cmd = turn_controller.get_command_final_turn(pose, target);
                    // }
                    // else
                    // {   
                    //     printf("using SMART controller\n");
                    //     state_ == SMART;    
                    //     cmd = smart_controller.get_command(pose, target);
                    // }
                }
...
int main(int argc, char** argv)
{
    ...
    while(true)
    {
    	...
    	if(controller.timesync_initialized()){
            ...
            // Fwd vel
            if (cmd.vx > 0.25) cmd.vx = 0.25;
            else if (cmd.vx < -0.25) cmd.vx = -0.25;
            ...
            // if (cmd.wz < 0.4) cmd.wz = max_ang_vel /2.;
            // else if (cmd.wz > -0.4) cmd.wz = -max_ang_vel/2.;
