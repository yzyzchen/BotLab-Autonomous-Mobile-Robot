...
int8_t Exploration::executeExploringMap(bool initialize)
{
    ...
    */
    ...
    float distThreshold = 0.06; //need to tweak this thing
    float currDist;
    int unreachable;

    if (currentTarget_.x != 0 || currentTarget_.y != 0){
        currDist = std::hypot(currentPose_.x - currentTarget_.x, currentPose_.y - currentTarget_.y); //euclidean distance
    } else{
        currDist = 0;
    }
    std::cout<<"current distance to target: "<<currDist<<std::endl;

    if ((currDist <= distThreshold && !frontiers_.empty())) {
        auto frontierPath_ = plan_path_to_frontier(frontiers_, currentPose_, currentMap_, planner_);
        currentPath_ = frontierPath_.path_selected;
        // currentPath_.path.pop_back();
        // currentPath_.path_length = currentPath_.path.size();
        unreachable = frontierPath_.num_unreachable_frontiers;

        std::cout<<"number of unreachable_frontiers: "<<frontierPath_.num_unreachable_frontiers<<std::endl;
        // std::cout<<"current path:"<<currentPath_.path<<std::endl;
    ...
    if(frontiers_.empty() || (frontiers_.size() == unreachable))
    {
        status.status = mbot_lcm_msgs::exploration_status_t::STATUS_COMPLETE;
    }
...
int8_t Exploration::executeReturningHome(bool initialize)
{
    ...
    */
    ...
    planner_.setMap(currentMap_);
    int8_t fail_count = 0;
    std::cout<<"current pose"<<currentPose_.x<<","<<currentPose_.y<<"; home pose"<<homePose_.x<<","<<homePose_.y<<std::endl;

    if (!hasReturnHomePath_) {
        currentPath_ = planner_.planPath(currentPose_, homePose_);
        while (!planner_.isPathSafe(currentPath_) && fail_count < 10) {
            planner_.setMap(currentMap_);
            currentPath_ = planner_.planPath(currentPose_, homePose_);
            fail_count++;
        }
